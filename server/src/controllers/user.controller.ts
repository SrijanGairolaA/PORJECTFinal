...
import { User, UserDemographic } from "@prisma/client";
...
const setUserDemographicDetails = async(req:Request ,res:Response): Promise<Response | void>=>{
   try {
     const { id } = req.params;
    ...
    const newData: UserDemographic = await prisma.userDemographic.create({
        data:{
             userId: userId,
             gender,
             age,
